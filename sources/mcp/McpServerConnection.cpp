...
bool providerSupportsTools(Providers::Provider *p)
{
    if (!p)
        return false;
    return p->capabilities().testFlag(Providers::ProviderCapability::Tools);
}
...
void McpServerConnection::setProviders(const QList<Providers::Provider *> &providers)
{
    m_providers.clear();
