...
{
    Templates::PromptTemplate *templ = m_templateManger.getFimTemplateByName(templateAspect.value());
...
ConfigurationManager::ConfigurationManager(QObject *parent)
    ...
    , m_providersManager(Providers::ProvidersManager::instance())
    , m_templateManger(Templates::PromptTemplateManager::instance())
{}
...
void ConfigurationManager::selectModel()
{
    ...
    if (auto provider = m_providersManager.getProviderByName(providerName)) {
        if (!provider->capabilities().testFlag(Providers::ProviderCapability::ModelListing)) {
