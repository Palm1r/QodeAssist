...
class OpenAIToolsManager : public QObject
{
    Q_OBJECT

public:
    explicit OpenAIToolsManager(QObject *parent = nullptr);

    void setToolsFactory(Tools::ToolsFactory *toolsFactory);

    // Process SSE events - returns text for user
...
private:
    ...
    struct RequestState
    {
        ...
        QHash<int, ToolCall> activeCalls;    // index -> ToolCall
        ...
        bool allToolsReceived = false;       // All tool calls received from stream
        ...
        {
            return allToolsReceived && pendingToolIds.isEmpty() && currentExecutingToolId.isEmpty();
        }
