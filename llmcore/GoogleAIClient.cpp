    ...
    const RequestID &requestId, const QUrl &url, const QJsonObject &payload)
{
    ...
    LOG_MESSAGE(
        QString("GoogleAIClient: Sending request %1 to %2").arg(requestId, url.toString()));
...
{
    LOG_MESSAGE(QString("GoogleAIClient: Cancelling request %1").arg(requestId));
...
{
    if (error) {
        LOG_MESSAGE(QString("GoogleAIClient request %1 failed: %2").arg(requestId, *error));
