...
{
    if (!tool) {
        qCDebug(llmToolsLog).noquote() << "Attempted to add null tool";
        return;
    }

    const QString toolName = tool->name();
    if (m_tools.contains(toolName)) {
        qCDebug(llmToolsLog).noquote() << QString("Tool '%1' already registered, replacing").arg(toolName);
    }
...
{
    qCDebug(llmToolsLog).noquote() << QString("Queueing tool %1 (ID: %2) for request %3")
    ...
    for (const auto &tool : queue.queue) {
        if (tool.id == toolId) {
            qCDebug(llmToolsLog).noquote() << QString("Tool %1 already in queue for request %2").arg(toolId, requestId);
            return;
        }
    }

    if (queue.completed.contains(toolId)) {
        qCDebug(llmToolsLog).noquote() << QString("Tool %1 already completed for request %2").arg(toolId, requestId);
        return;
    }
    ...
    qCDebug(llmToolsLog).noquote() << QString("Tool %1 added to queue (position %2)")
                    .arg(toolName)
...
void ToolsManager::executeNextTool(const QString &requestId)
{
    ...
    if (queue.queue.isEmpty()) {
        qCDebug(llmToolsLog).noquote() << QString("All tools complete for request %1, emitting results")
                        .arg(requestId);
    ...
    qCDebug(llmToolsLog).noquote() << QString("Executing tool %1 (ID: %2) for request %3 (%4 remaining)")
                    .arg(pendingTool.name, pendingTool.id, requestId)
                    .arg(queue.queue.size());

    BaseTool *toolInstance = m_tools.value(pendingTool.name);
    if (!toolInstance) {
        qCDebug(llmToolsLog).noquote() << QString("Tool not found: %1").arg(pendingTool.name);
    ...
    qCDebug(llmToolsLog).noquote() << QString("Started async execution of %1").arg(pendingTool.name);
}
