    ...
    const QJsonObject &input)
{
    ...
    qCDebug(llmToolsLog).noquote() << QString("Starting tool execution: %1 (ID: %2)").arg(tool->name(), toolId);
...
void ToolHandler::cleanupRequest(const QString &requestId)
{
    ...
    while (it != m_activeExecutions.end()) {
        if (it.value()->requestId == requestId) {
            ...
            qCDebug(llmToolsLog).noquote() << QString("Canceling tool %1 for request %2").arg(execution->toolName, requestId);
...
void ToolHandler::onToolExecutionFinished(const QString &toolId)
{
    ...
    try {
        ...
        qCDebug(llmToolsLog).noquote() << QString("Tool %1 failed: %2").arg(execution->toolName, error);
