...
QFuture<QList<QString>> OpenAIResponsesClient::listModels(const QString &baseUrl)
{
    ...
    return m_httpClient->get(request)
        ...
        .onFailed([](const std::exception &e) {
            qCDebug(llmOpenAILog).noquote() << QString("Error fetching OpenAI models: %1").arg(e.what());
    ...
    if (m_dataBuffers.contains(requestId)) {
        ...
        if (!buffers.responseContent.isEmpty()) {
            ...
            qCDebug(llmOpenAILog).noquote()
                << QString("WARNING: Response content is empty for %1, emitting empty response")
                       .arg(requestId);
            emit fullResponseReceived(requestId, "");
        }
    } else {
        qCDebug(llmOpenAILog).noquote()
            << QString("WARNING: No data buffer found for %1").arg(requestId);
    }
    ...
    const QString &requestId, const QHash<QString, QString> &toolResults)
{
    if (!m_messages.contains(requestId) || !m_requestUrls.contains(requestId)) {
        qCDebug(llmOpenAILog).noquote()
            << QString("ERROR: Missing data for continuation request %1").arg(requestId);
        cleanupRequest(requestId);
        return;
    }
    ...
    if (eventType == "response.content_part.added") {
        ...
        if (!item.isEmpty() && item["type"].toString() == "reasoning") {
            ...
            if (!callId.isEmpty()) {
                ...
                qCDebug(llmOpenAILog).noquote()
                    << QString("ERROR: Function call done but call_id is empty");
            }
        }
        ...
        qCDebug(llmOpenAILog).noquote()
            << QString("WARNING: Unhandled event type '%1' for request %2\nData: %3")
                   .arg(eventType)
                   .arg(requestId)
