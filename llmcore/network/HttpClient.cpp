...
{
    qCDebug(llmNetworkLog).noquote() << QString("GET %1").arg(request.url().toString());
...
QFuture<QByteArray> HttpClient::post(const QNetworkRequest &request, const QJsonObject &payload)
{
    ...
    qCDebug(llmNetworkLog).noquote() << QString("POST %1, data: %2")
                                    ...
                                    std::optional<QJsonObject> payload)
{
    ...
    if (payload) {
        QJsonDocument doc(*payload);
        qCDebug(llmNetworkLog).noquote() << QString("DELETE %1, data: %2")
                        .arg(request.url().toString(), doc.toJson(QJsonDocument::Indented));
        reply = m_manager->sendCustomRequest(request, "DELETE", doc.toJson(QJsonDocument::Compact));
    } else {
        qCDebug(llmNetworkLog).noquote() << QString("DELETE %1").arg(request.url().toString());
                                        ...
                                        std::shared_ptr<QPromise<QByteArray>> promise)
{
    connect(reply, &QNetworkReply::finished, this, [this, reply, promise]() {
        ...
        qCDebug(llmNetworkLog).noquote() << QString("Non-streaming request - HTTP Status: %1").arg(statusCode);
        ...
        if (hasError) {
            ...
            qCDebug(llmNetworkLog).noquote() << QString("Non-streaming request - Error: %1").arg(errorMsg);
...
{
    QJsonDocument doc(payload);
    qCDebug(llmNetworkLog).noquote() << QString("POST streaming %1, data: %2")
                    .arg(request.url().toString(), doc.toJson(QJsonDocument::Indented));
...
void HttpClient::cancelRequest(const QString &requestId)
{
    ...
    if (it != m_activeRequests.end()) {
        ...
        qCDebug(llmNetworkLog).noquote() << QString("Cancelled request: %1").arg(requestId);
    }
}
...
void HttpClient::onStreamingFinished()
{
...

    {
        ...
        qCDebug(llmNetworkLog).noquote() << QString("Request %1 - HTTP Status: %2").arg(requestId).arg(statusCode);

        if (!responseBody.isEmpty()) {
            qCDebug(llmNetworkLog).noquote() << QString("Request %1 - Response body (%2 bytes): %3")
                            .arg(requestId)
        ...
        if (error) {
            qCDebug(llmNetworkLog).noquote() << QString("Request %1 - Error: %2").arg(requestId, *error);
        }
    }
...
void HttpClient::addActiveRequest(QNetworkReply *reply, const QString &requestId)
{
    ...
    qCDebug(llmNetworkLog).noquote() << QString("Added active request: %1").arg(requestId);
}
