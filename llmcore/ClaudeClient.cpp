    ...
    const RequestID &requestId, const QUrl &url, const QJsonObject &payload)
{
    ...
    LOG_MESSAGE(QString("ClaudeClient: Sending request %1 to %2").arg(requestId, url.toString()));
...
{
    LOG_MESSAGE(QString("ClaudeClient: Cancelling request %1").arg(requestId));
...
{
    if (error) {
        LOG_MESSAGE(QString("ClaudeClient request %1 failed: %2").arg(requestId, *error));
