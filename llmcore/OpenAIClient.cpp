    ...
    const RequestID &requestId, const QUrl &url, const QJsonObject &payload)
{
    ...
    LOG_MESSAGE(
        QString("OpenAIClient: Sending request %1 to %2").arg(requestId, url.toString()));
...
{
    LOG_MESSAGE(QString("OpenAIClient: Cancelling request %1").arg(requestId));
...
{
    if (error) {
        LOG_MESSAGE(QString("OpenAIClient request %1 failed: %2").arg(requestId, *error));
    ...
    const QString &requestId, const QHash<QString, QString> &toolResults)
{
    ...
    LOG_MESSAGE(QString("Tool execution complete for request %1").arg(requestId));
...
void OpenAIClient::handleMessageComplete(const QString &requestId)
{
    ...
    if (message->state() == MessageState::RequiresToolExecution) {
        LOG_MESSAGE(QString("Message requires tool execution for %1").arg(requestId));
        ...
        LOG_MESSAGE(QString("Message marked as complete for %1").arg(requestId));
    }
}

void OpenAIClient::cleanupRequest(const RequestID &requestId)
{
    LOG_MESSAGE(QString("Cleaning up request %1").arg(requestId));
