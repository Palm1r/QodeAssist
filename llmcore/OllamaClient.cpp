...
QFuture<QList<QString>> OllamaClient::listModels(const QString &baseUrl)
{
    ...
    return m_httpClient->get(request)
        ...
        .onFailed([](const std::exception &e) {
            LOG_MESSAGE(QString("Error fetching Ollama models: %1").arg(e.what()));
    ...
    const RequestID &requestId, const QUrl &url, const QJsonObject &payload)
{
    ...
    LOG_MESSAGE(
        QString("OllamaClient: Sending request %1 to %2").arg(requestId, url.toString()));
...
{
    LOG_MESSAGE(QString("OllamaClient: Cancelling request %1").arg(requestId));
...
{
    if (error) {
        LOG_MESSAGE(QString("OllamaClient request %1 failed: %2").arg(requestId, *error));
...
void OllamaClient::processStreamData(const QString &requestId, const QJsonObject &data)
{
    ...
    if (data.contains("thinking")) {
        ...
        if (!thinkingDelta.isEmpty()) {
            ...
            LOG_MESSAGE(QString("OllamaClient: Received thinking delta, length=%1")
    ...
    if (data.contains("message")) {
        ...
        if (messageObj.contains("tool_calls")) {
            ...
            LOG_MESSAGE(
                QString("OllamaClient: Found %1 structured tool calls").arg(toolCalls.size()));
    ...
    if (data["done"].toBool()) {
        if (data.contains("signature")) {
            ...
            LOG_MESSAGE(QString("OllamaClient: Set thinking signature, length=%1")
